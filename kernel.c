...
void k_init(uint8_t maxTasks, uint32_t tickRate)
{
    ...
    // Set up the system clock to 20 MHz
    ...
    // Set up the period for the SysTick timer as a function of the system clock
    SysTickPeriodSet (SysCtlClockGet() / tickRate);
...
/*******************************************************/
taskId_t k_register_task (void (*taskEnter) (void), uint8_t priority, uint8_t freq)
{
    ...
    if (num_registered_tasks < scheduler_max_tasks) {
        ...
        sortTasks (); // reorder tasks in descending priority
...
void k_start (void)
{
    ...
    while (true) { // infinite loop
        uint8_t i = 0;
        bool handled = false; // true if task handled

        // On each iteration, execute the highest priority task marked as ready
        while (i < num_registered_tasks && !handled) { // scheduled_task[] elements in priority order
...
void k_notify_task (taskId_t taskId)
{
    ...
    Task_t *myTask = getTask (taskId); // retrieve task from taskId
...
/*******************************************************
 * Support functions: sortTasks, generateId, getTask
 *******************************************************/
void sortTasks (void)
{
  /* sort elements in descending order of priority */
  int i, j;
  Task_t tmp;

  // iterate over array to arrange elements in descending order
  for (i = 0; i < num_registered_tasks; i++) {
        for (j = i+1; j < num_registered_tasks; j++) {
            if (scheduled_task[i].priority > scheduled_task[j].priority) { // swap positions
                tmp = scheduled_task[i];
                scheduled_task[i] = scheduled_task[j];
                scheduled_task[j] = tmp;
            }
        }
    }
}
...
/*******************************************************/
Task_t *getTask (taskId_t taskId)
{
